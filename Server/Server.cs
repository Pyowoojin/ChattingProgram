    ...
    private async void RunAsync(object? sender)
    {
        ...
        try
        {
            while (true)
            {
                ...
                /* t1, t2 중 먼저 끝나는 Task 반환
                 * 만약 t1이 응답이 없다면 t2가 먼저 끝날 것임.
                 * 그러므로 연결을 종료시킨다.
                 */
                ...
                if (packetType == PacketType.LoginRequest)
                {
                    ...
                    // 중복 접속 방지
                    Clients.AddOrUpdate(packet.Id, clientSocket, (oldKey, oldValue) =>
					{
                ...
                else if (packetType == PacketType.EnterRoomRequest)
                {
                    ...
                    if (Rooms.TryGetValue(packet.RoomName, out var room))
                    {
                        roomName = packet.RoomName;
                        room.Users.TryAdd(id, nickname);
                        Console.WriteLine($"{packet.RoomName} : {nickname} 입장 성공");
                    ...
                    {
                        Console.WriteLine($"{packet.RoomName} : {nickname} 입장 실패");
